...
import es.nicolas.auth.exceptions.AuthDifferentPasswords;
import es.nicolas.auth.exceptions.AuthExistingUsernameOrEmail;
import es.nicolas.auth.exceptions.AuthSignInNotValid;
import es.nicolas.auth.repositories.AuthUsersRepository;
import es.nicolas.auth.services.jwt.JwtService;
import es.nicolas.user.models.Role;
    ...
    public JwtAuthResponse signUp(UserSignUpRequest request) {
        ...
        if(request.getPassword().contentEquals(request.getPasswordComprobacion())){
            ...
            try {
                ...
                throw new AuthExistingUsernameOrEmail("El usuario con username " + request.getUsername() + " ya existe");
            }
        } else {
